...
public class DatabaseStreamStreamJoinIntegrationTest extends AbstractDatabaseJoinIntegrationTest {
    private KStream<Long, String> leftStream;
    private KStream<Long, String> rightStream;

    public DatabaseStreamStreamJoinIntegrationTest(boolean cacheEnabled) {
        super(cacheEnabled);
    }

    @Before
    public void prepareTopology() throws InterruptedException, SQLException {
        super.prepareEnvironment();

        appID = "stream-stream-join-integration-test";

        builder = new StreamsBuilder();
        leftStream = builder.<SchemaAndValue, SchemaAndValue>stream(INPUT_TOPIC_LEFT).map((SchemaAndValue k, SchemaAndValue v) -> {
            Struct struct = ((Struct) v.value());
            Object field = struct != null ? struct.get("value") : null;
            String value = field != null ? field.toString() : null;
            return new KeyValue<Long, String>(anyUniqueKey, value);
        });
        rightStream = builder.<SchemaAndValue, SchemaAndValue>stream(INPUT_TOPIC_RIGHT).map((SchemaAndValue k, SchemaAndValue v) -> {
            Struct struct = ((Struct) v.value());
            Object field = struct != null ? struct.get("value") : null;
            String value = field != null ? field.toString() : null;
            return new KeyValue<Long, String>(anyUniqueKey, value);
    ...
    public void testInner() throws Exception {
        ...
        leftStream.join(rightStream, valueJoiner, JoinWindows.of(10000), Joined.with(Serdes.Long(), Serdes.String(), Serdes.String()))
                .map((key, value) -> {
                    Schema schema = SchemaBuilder.struct().name("word")
                            .field("value", Schema.STRING_SCHEMA).build();
                    Struct struct = new Struct(schema).put("value", value);
    ...
    public void testInnerRepartitioned() throws Exception {
        ...
        leftStream.map(MockMapper.<Long, String>noOpKeyValueMapper())
                .join(rightStream.flatMap(MockMapper.<Long, String>noOpFlatKeyValueMapper())
                        ...
                        valueJoiner, JoinWindows.of(10000), Joined.with(Serdes.Long(), Serdes.String(), Serdes.String()))
                .map((key, value) -> {
                    Schema schema = SchemaBuilder.struct().name("word")
                            .field("value", Schema.STRING_SCHEMA).build();
                    Struct struct = new Struct(schema).put("value", value);
    ...
    public void testLeft() throws Exception {
        ...
        leftStream.leftJoin(rightStream, valueJoiner, JoinWindows.of(10000), Joined.with(Serdes.Long(), Serdes.String(), Serdes.String()))
                .map((key, value) -> {
                    Schema schema = SchemaBuilder.struct().name("word")
                            .field("value", Schema.STRING_SCHEMA).build();
                    Struct struct = new Struct(schema).put("value", value);
    ...
    public void testOuter() throws Exception {
        ...
        leftStream.outerJoin(rightStream, valueJoiner, JoinWindows.of(10000), Joined.with(Serdes.Long(), Serdes.String(), Serdes.String()))
                .map((key, value) -> {
                    Schema schema = SchemaBuilder.struct().name("word")
                            .field("value", Schema.STRING_SCHEMA).build();
                    Struct struct = new Struct(schema).put("value", value);
    ...
    public void testMultiInner() throws Exception {
        ...
        leftStream.join(rightStream, valueJoiner, JoinWindows.of(10000), Joined.with(Serdes.Long(), Serdes.String(), Serdes.String()))
                .join(rightStream, valueJoiner, JoinWindows.of(10000), Joined.with(Serdes.Long(), Serdes.String(), Serdes.String()))
                .map((key, value) -> {
                    Schema schema = SchemaBuilder.struct().name("word")
                            .field("value", Schema.STRING_SCHEMA).build();
                    Struct struct = new Struct(schema).put("value", value);
